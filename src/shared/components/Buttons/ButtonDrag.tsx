...
export const DraggableButton: React.FC<DraggableButtonProps> = ({ handleToggleSidebar, icon, hasNewNotification }) => {
  const [startPos, setStartPos] = useState<{ x: number; y: number }>({ x: 0, y: 0 });
  const [position, setPosition] = useState<{ x: number; y: number }>({ x: 0, y: 0 });
  const [bounds, setBounds] = useState({ top: 10, left: 0, right: 0, bottom: 0 });
  ...
  const handleStop = (e: DraggableEvent, data: DraggableData) => {
    ...
    const snapX = data.x < screenWidth / 2 ? 10 : screenWidth - buttonWidth - 10;
  ...
  return (
    <Draggable bounds={bounds} position={position} onStart={handleStart} onStop={handleStop}>
