...
const RequestHistoryCard = React.memo(({ request, onViewDetails }: RequestHistoryCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className='flex justify-between items-center pb-3 space-x-2'>
        ...
        <div className='flex-shrink-0'>{RequestStatusBadge(request.status)}</div>
      </div>
      ...
      {(request.type === RequestType.STAFF || request.type === RequestType.ORDER) && request.rejectReason && (
        <div className='pt-3 overflow-hidden break-words'>
          <h4 className='font-semibold overflow-hidden break-words'>Lý do</h4>
          <p className='text-sm text-black bg-gray-50 rounded-md py-1 px-1'>{request.rejectReason}</p>
        </div>
      )}
